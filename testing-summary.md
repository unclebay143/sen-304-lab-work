...
- **Target:** `grades_final_code.py` (imported in `test_grades_final.py`).
...
Output:
![Testing summary](screenshots/test-report-2.png)


Optional coverage for the module under test:

```bash
pytest -v test_grades_final.py --cov=grades_final_code --cov-report=term-missing
```
